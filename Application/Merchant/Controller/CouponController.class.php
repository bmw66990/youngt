...
class CouponController extends CommonController {
    ...
    public function index($partner_id = '') {
        ...
        $now_time = time();
        // 查询项目
        $field = 'id,product,title,begin_time,end_time,now_number,max_number,pre_number,team_price,ucaii_price,state,team_type';
        $count = $team->where($where)->count();
        $Page = $this->pages($count, $this->reqnum);
        $list = $team->field($field)
                ->where($where)->order('end_time desc')
        ...
        if($list){
            ...
            foreach($list as &$v){
               ...
               if(intval($v['max_number'])>0 && intval($v['now_number']) >= intval($v['max_number'])){
                    $v['status'] = 4; // 已卖完
               }
               
               if(isset($v['team_type']) && $v['team_type']=='unaudited' && isset($v['end_time']) && $v['end_time'] > $now_time){
                   $v['status'] = 5; // 待审核
               }
        ...
        $voucher_data  = M('points_team')->field('id,name,consume_num,limit_num,begin_time,end_time')
                                        ->where(array('partner_id' => $partner_id))->order('id desc')->limit(10)
                                        ->select();
        ...
        //券号验证撤销
        //2015.3.6superegoliu加 输出可以撤销的券号
        $coupon = M('coupon');
        $time=strtotime(date('Y-m-d'));
        $end_time=$time+86400;
        $sql="SELECT `coupon`.consume_time,`team`.title,`coupon`.id FROM `coupon`  left outer  join `partner_income` on  `coupon`.id=`partner_income`.coupon_id left outer  join `team` on `coupon`.team_id=`team`.id where `coupon`.consume='Y' and `partner_income`.pay_id=0 and `coupon`.consume_time >= {$time} and `coupon`.consume_time <= {$end_time} and `coupon`.partner_id={$partner_id} order by  `coupon`.consume_time desc limit 0,5";
        $Model = M();
        $cxcoupons=$Model->query($sql);
        if($cxcoupons){
            $this->assign('cxcoupon',$cxcoupons);
        }
        //
        $this->assign($data);
        $this->display();
    ...
    public function indexExecuteCheckCoupon() {
        ...
        $couponCopy = array();
        $couponRes = array();
        $field = 'id,consume,team_id,order_id,consume_time,partner_id,team_price,ucaii_price';
    ...
    public function team() {
        ...
        $this->assign($data);
        $this->display('Coupon/team_detail');
    }
    ...
    public function cxCoupon()
    {
        ...
        if($qd=='y'){
            if($id){
                ...
                $udcoupon=M('coupon')->where($where)->save(array('consume'=>'N'));
                $delpartner_income=M('partner_income');
                $wheres['coupon_id']=$id;
                $wheres['partner_id']=$this->partner_id;
                $delpartner_income->where($wheres)->delete();
            }
        }else{
            $sql="SELECT `coupon`.consume_time,`team`.title,`coupon`.id FROM `coupon` left outer  join `team` on `coupon`.team_id=`team`.id where `coupon`.id={$id}";
            $Model = M();
            $getcouponteam=$Model->query($sql);
            $this->assign('getcouponteam',$getcouponteam);
            $this->display();
        }
