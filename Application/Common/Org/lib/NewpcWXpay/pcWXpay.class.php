...
class PcWXpay {
     ...
     * @return string
     */
    public function createWXpay($product_id) {
        $result = $this->wx_pay_native->GetPrePayUrl($product_id);
    ...
    public function createWXpayCode($payId, $title, $product, $payFee, $plat, $notify_url) {
        ...
        $this->wx_pay_unified_order->SetTime_expire(date("YmdHis", time() + 3600));
    ...
    public function doPay($data, $option) {
        $openId = $this->wx_pay_js->GetOpenid();
