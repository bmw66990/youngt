...
class chinaBank 
{
	...
	function verify($data, $sign) {
		...
		$verify = openssl_verify($data, $this->hex2bins($sign), $p);
		openssl_free_key($p);
		return $verify > 0;
	}
	...
	function order($datas, $type = 'NoticePay', $exp = 0) {
		...
		file_put_contents('qingqiu.txt', $str);
...
/**
	...
	function seek() {
		...
		$data = array(
			...
			'OrderDate' => date('Ymd', '1420987603'),//订单日期
			'MerOrderId' => 'go-695631-1-rvmo',//商户订单号
			'TransId' => '662015011108371729',//交易流水
			'MerId' => $this->merchantId, //商户号
			'MerTermId' => $this->teamId, //终端号
			'Reserve'   => '',
			'MerSign' => ''
		);
		...
		//发送请求
		$result = $this->post($data,'https://mpos.quanminfu.com:6004/merFrontMgr/orderQueryServlet');
		header('Content-type: text/html; charset=utf-8');
		print "<pre>";
		print_r($result);
		print "</pre>";
