...
class ReviewController extends CommonController {
    ...
    public function index() {
        ...
        $teamId = I('post.team_id', 0, 'intval');
        ...
        $field = array('comment.team_id,comment.user_id,comment.order_id,comment.comment_num,comment.content,comment.partner_content,team.product,user.username');
    ...
    public function review() {
        ...
        $result = $comment->where('order_id =' . $id)->save(array('partner_content' => trim($content)));
        if (!$result) {
            $this->ajaxReturn(array('code' => -1, 'error' => '评论失败！'));
        }
