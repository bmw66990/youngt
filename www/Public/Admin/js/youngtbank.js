$(function() {
    ...
    $('span#refund-deal').die().live('click', function() {
        ...
        var coupon_ids = $("#pay_id").val();
        var bank_user = $("#partner_bank_user").val();
        var bank_no = $("#partner_bank_no").val();
        var bank_name = $("#partner_bank_name").val();
        var refund_type = $("#refund-type").val();//$parent.find('select#refund-type').val();
        var href = $(this).attr('load_href');
        if ($this.hasClass('disabled')) {
            return false;
        }
        ...
        $this.addClass('disabled');
        $this.html('正在处理...');
        $.post(href, {refund_type: refund_type, id: coupon_ids, partner_bank_user: bank_user, partner_bank_no: bank_no, partner_bank_name: bank_name}, function(res) {
            $this.removeClass('disabled');
            $this.html('确定');
            if (res.code && res.code != 0 && res.error) {
                parent.window.alert(res.error);
